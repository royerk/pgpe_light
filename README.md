...
class Driver:
    def __init__(self, my_population_size):
        self.agent = [Agent() for _ in range(my_population_size)]
        self.solver = PGPE(
            solution_length=Agent().get_needed_weights(),
            popsize=my_population_size,
            optimizer="clipup",
            optimizer_config=dict(max_speed=0.25, momentum=0.9),  # .25 .9
            center_learning_rate=0.1,  # 0.15,
