...
def make_vector(
    x: Union[Real, Iterable[Real]], length: Integral, dtype=Union[str, np.dtype]
) -> np.ndarray:
...
def non_negative_int(x) -> int:
    ...
    if x < 0:
        raise ValueError(
            "Expected a non-negative integer, but received: " + repr(org_x)
        )
    return x
...
def fill_parameters(net: nn.Module, vector: torch.Tensor):
    ...
    for p in net.parameters():
        ...
        address += n
    assert address == len(vector), "This vector is larger than expected"
