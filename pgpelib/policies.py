...
class Policy:
    ...
    def __init__(
        ...
        *,
        ...
        self._observation_space, self._action_space = get_env_spaces(
            self._env_name, self._env_config
        )
    ...
    def _use_policy(self, observation: Iterable[Real]) -> Action:
        ...
        if isinstance(self._action_space, Box):
            ...
            raise TypeError(
                "Cannot work with this action space: " + repr(self._action_space)
            )
        return action
    ...
    def _run_from_dict(
        ...
        *,
        ...
        for policy_key, policy_params in policy_param_dict.items():
            results[policy_key] = self.set_params_and_run(
                policy_params,
                decrease_rewards_by=decrease_rewards_by,
                max_episode_length=max_episode_length,
            )
    ...
    def set_params_and_run_all(
        self,
        policy_params_all: Union[List[ParamVector], Dict[Any, ParamVector]],
        *,
        decrease_rewards_by: Real = 0.0,
        max_episode_length: Optional[Integral] = None
    ) -> Union[List[Tuple[float, int]], Dict[Any, Tuple[float, int]]]:
        ...
        received_dict = hasattr(policy_params_all, "keys") and hasattr(
            policy_params_all, "values"
        )
    ...
    def set_main_obs_stats(self, obs_stats: RunningStat):
        ...
        if obs_stats is None:
            raise ValueError("The main observation stats cannot be given as None.")
...
class MLPPolicy(Policy):
    ...
    ACTIVATION_CLS = {"tanh": nn.Tanh, "relu": nn.ReLU}
