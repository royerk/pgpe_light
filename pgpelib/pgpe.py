...
def compute_reinforce_update(
    ...
    for i_direction in range(ndirs):
        ...
        total_sigma += (avg_dir_score - baseline) * (
            ((scaled_noise**2.0) - stdev_sq) / stdev
        )
...
class PGPE:
    ...
    def tell(
        ...
        interactions: Optional[Iterable[Integral]] = None,
    ) -> bool:
        ...
        if iteration_is_done:
            ...
            if self._stdev_max_change is not None and self._stdev_max_change > 0:
                ...
                np.clip(
                    self._stdev, stdev_min_allowed, stdev_max_allowed, out=self._stdev
                )
    ...
    def __getitem__(
        ...
        if isinstance(i, slice):
            result = []
            for j in range(
                fix_index(i.start, 0), fix_index(i.stop, len(self._solutions)), i.step
            ):
